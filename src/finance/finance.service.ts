...
export class FinanceService {
  ...
  changeData(arrayData) {
    return new Promise((resolve) => {
      ...
      const jumlah: Tanggal = {};

      arrayData.forEach((item) => {
        jumlah[item.tanggal]
          ? jumlah[item.tanggal]++
          : (jumlah[item.tanggal] = 1);
        newData[item.tanggal] = arrayData.filter(
          (itemFilter) => itemFilter.tanggal === item.tanggal,
        );
      });

      const objectArray: Array<Array<string | Array<Data>>> =
